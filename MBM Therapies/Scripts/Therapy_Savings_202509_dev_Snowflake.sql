/*==============================================================================
 * CHANGE LOG
 * iff classification of PT/OP Rehab/ Chiro to case when
 * Change syntax to be more Snowflake-compliant and efficient
 *==============================================================================*/
...
drop table if exists tmp_1m.kn_mbm_dtl_202509;					
create table tmp_1m.kn_mbm_dtl_202509 stored as orc as 					
select 		
	fin_mbi_hicn_fnl			
	, fin_inc_month			
	, fin_inc_qtr 		
...
select count(*) from tmp_1m.kn_mbm_dtl_202509;  -- 430370488 415060257  407463331 399836379   392256742 (removed 2019) 442855070  435558291  428264965  450161568
...
drop table if exists tmp_1m.kn_mbm_mshp_202509; 							
create temporary table tmp_1m.kn_mbm_mshp_202509 stored as orc as 							
select 							
...
from tmp_1m.kn_mbm_dtl_202509 as a
where global_cap = 1
group by 							
	fin_inc_month							
...
select count(*) from tmp_1m.kn_mbm_mshp_202509; --9743  9606

drop table if exists tmp_1m.kn_mbm_mshp_sum1_202509;
create table tmp_1m.kn_mbm_mshp_sum1_202509 stored as orc as
select 
	'MM' as data_type
	, ep_start_mo
	, '' as visit_mo
	...
	, 0 as allowed_amt
	, sum(mm) as mms
from tmp_1m.kn_mbm_mshp_202509
...
select count(*) from tmp_1m.kn_mbm_mshp_sum1_202509; -- 134 132 128 126 124  144 142

select * from tmp_1m.kn_mbm_mshp_sum1_202509;
...
describe tmp_1y.pa_trckng_op_evnt_lopa_dtl;
describe tmp_1y.pa_trckng_pr_evnt_lopa_dtl;

drop table if exists tmp_1m.kn_lopa_op_1_202509;
create table tmp_1m.kn_lopa_op_1_202509 as
select  
...
select count(*) from tmp_1m.kn_lopa_op_1_202509; -- 2484707 2417275  2306199  2164101  1729931 1526791   1338831

drop table if exists tmp_1m.kn_lopa_op_202509;
create table tmp_1m.kn_lopa_op_202509 as
select 
	case when still_lopa_mbi_dos is not null or overturn_lopa_mbi_dos is not null then mbi_dos else null end as ever_lopa
	, *
from tmp_1m.kn_lopa_op_1_202509
; 

select count(*) from tmp_1m.kn_lopa_op_202509; -- 2417275  2306199 2164101

drop table if exists tmp_1m.kn_lopa_pr_1_202509;
create table tmp_1m.kn_lopa_pr_1_202509 as
select  
...
select count(*) from tmp_1m.kn_lopa_pr_1_202509; -- 3714719  3501938 3263702

drop table if exists tmp_1m.kn_lopa_pr_202509;
create table tmp_1m.kn_lopa_pr_202509 as
select 
	case when still_lopa_mbi_dos is not null or overturn_lopa_mbi_dos is not null then mbi_dos else null end as ever_lopa
	, *
from tmp_1m.kn_lopa_pr_1_202509
;

select count(*) from tmp_1m.kn_lopa_pr_202509; -- 3714719  3501938 3263702
		
/*==============================================================================
 * PROFESSIONAL CLAIMS PROCESSING
 * Pull in PR claims, combine with LOPA flags
 *==============================================================================*/
drop table if exists tmp_1m.kn_mbm_episode_pr_202509; 
create table tmp_1m.kn_mbm_episode_pr_202509 stored as orc as
select			
...
left join tmp_1m.kn_lopa_pr_202509 as b		   
...
select count(*) from tmp_1m.kn_mbm_episode_pr_202509; -- 64354173  62711063  60990327
...
select *
	, max(iff(instr(clm_rev_rsn_1_10,'00473-') > 0 , 1,0)) over (partition by site_cd,clm_aud_nbr,sbscr_nbr) as clm_apc_flg
...
drop table if exists tmp_1m.kn_mbm_episode_op_202509; 
create table tmp_1m.kn_mbm_episode_op_202509 as 
select			
	a.gal_mbi_hicn_fnl as mbi		
	, a.component		
	, a.eventkey as id		
...
left join tmp_1m.kn_lopa_op_202509 as b
	on concat(a.gal_mbi_hicn_fnl, "_", a.fst_srvc_dt) = b.total_mbi_dos 
    and a.proc_cd = b.proc_cd
    and a.prov_tin = b.prov_tin
...
select count(*) from tmp_1m.kn_mbm_episode_op_202509; -- 38657890  37706450  36737912  35885437
...
select * from tmp_1m.kn_mbm_episode_pr_202509
union all
select * from tmp_1y.kn_mbm_episode_pr_2018_2020
union all
select * from tmp_1m.kn_mbm_episode_op_202509
union all
...
drop table if exists tmp_1m.kn_mbm_episode_2_202509; 
create table tmp_1m.kn_mbm_episode_2_202509 as 			
select			
	*			
	, iff(dnl_allowed > 0.01, 'Paid', iff(still_lopa = 1, 'LOPA', iff(apc_pbl_flg = 1, 'APC-Paid', 'Other Denied' ))) as claim_status
	, iff(tin_num is null,0,1) as optum_flg 			
...
select count(*) from tmp_1m.kn_mbm_episode_2_202509; -- 134268620 141253070  138658520  135969246

describe  tmp_1m.kn_mbm_episode_2_202509;

select serv_month, sum(allowed) as allowedamt 
from tmp_1m.kn_mbm_episode_2_202509
...
drop table if exists tmp_1m.kn_mbm_episode_3_202509;  
create table tmp_1m.kn_mbm_episode_3_202509 as 
select			
...
select count(*) from tmp_1m.kn_mbm_episode_3_202509; -- 64545494 63423287 66929218  65797555     64650521

drop table if exists tmp_1m.kn_mbm_episode_4_202509;  
create table tmp_1m.kn_mbm_episode_4_202509 as  			
select 
	mbi
	, component
	, id
	...
	, vsts
	, adj_srvc_units
from tmp_1m.kn_mbm_episode_3_202509 as a
;

select count(*) from tmp_1m.kn_mbm_episode_4_202509; -- 64545494 66929218  65797555   64650521

drop table if exists tmp_1m.kn_mbm_episode_lag_202509 ;
create table tmp_1m.kn_mbm_episode_lag_202509 as 
select 
	*
...
select a.mbi
	...
	, iff(datediff(a.start_dt,b.start_dt) > 30, 1, 0) as ep_flag
	, a.i 
	, b.i as prev_i
	, a.serv_month
	, a.hce_qtr
	...
	, a.vsts
	, a.adj_srvc_units
from tmp_1m.kn_mbm_episode_4_202509 as a
left join tmp_1m.kn_mbm_episode_4_202509 as b 
	on a.mbi = b.mbi 
	and a.mbm_deploy_dt = b.mbm_deploy_dt
	and a.i = b.i+1 
;

select count(*) from tmp_1m.kn_mbm_episode_lag_202509; -- 64545494 66929218  65797555  64650521
...
drop table if exists tmp_1m.kn_mbm_episode_vst_ep_2_202509;  
create table tmp_1m.kn_mbm_episode_vst_ep_2_202509 as  
select a.mbi
	, a.component
	, a.id
	, a.start_dt
	...
	, a.adj_srvc_units
from (select *  
 		, sum(iff(prev_start_dt is null,1,ep_flag)) over (partition by mbi order by start_dt rows between unbounded preceding and current row) as cmltv_episodes 
	  from tmp_1m.kn_mbm_episode_lag_202509 ) as a
;

select count(*) from tmp_1m.kn_mbm_episode_vst_ep_2_202509; -- 66929218  65797555  64650521

drop table if exists tmp_1m.kn_mbm_episode_smry_202509;  
create table tmp_1m.kn_mbm_episode_smry_202509 as  
select  
...
from tmp_1m.kn_mbm_episode_vst_ep_2_202509 as a
group by 
	a.serv_month  
...
select count(*) from tmp_1m.kn_mbm_episode_smry_202509; -- 1555388  1526174  1496541
...
drop table if exists tmp_1m.kn_mbm_episode_ro_lag_202509;  
create table tmp_1m.kn_mbm_episode_ro_lag_202509 as  
select a.mbi
	, a.component
	, a.id
	, a.start_dt
	...
	, visit_dy_lag
	, iff(prev_start_dt is null,1,ep_flag) as ep_flag
	, ep_start_dt
	, cmltv_episodes
	, a.i 
	, a.prev_i
...
select count(*) from tmp_1m.kn_mbm_episode_ro_lag_202509; -- 66929218  65797555  64650521

drop table if exists tmp_1m.kn_mbm_episode_ro_lag2_202509;  
create table tmp_1m.kn_mbm_episode_ro_lag2_202509 as  
select 
	a.mbi
	, a.id
	, ep_start_dt
	...
	, allowed
	, 0 as mm 
from tmp_1m.kn_mbm_episode_ro_lag_202509 as a
;


select ep_start_mo, count(episodes) from tmp_1m.kn_mbm_episode_ro_lag2_202509
where episodes = 1
group by ep_start_mo

select count(*) from tmp_1m.kn_mbm_episode_ro_lag2_202509; -- 66929218  65797555  64650521

drop table if exists tmp_1m.kn_mbm_episode_agg6_ep_202509;  
create table tmp_1m.kn_mbm_episode_agg6_ep_202509 as  
select 
	...
	, 0 as allowed
	, 0 as mm 
from (select * from tmp_1m.kn_mbm_episode_ro_lag2_202509 where episodes = 1 ) as a
group by 
	ep_start_mo
...
select count(*) from tmp_1m.kn_mbm_episode_agg6_ep_202509; -- 45630  44961 44193  43381

/*==============================================================================
 * COMBINE VISITS AND EPISODES
 *==============================================================================*/

drop table if exists tmp_1m.kn_mbm_episode_agg6_202509;  
create table tmp_1m.kn_mbm_episode_agg6_202509 as  
select 
	'VISITS' as data_type
	, ep_start_mo 
	, concat(ep_start_year,'Q9') as ep_start_qtr
	...
	, sum(allowed) as allowed
	, 0 as mm 
from tmp_1m.kn_mbm_episode_ro_lag2_202509
group by 
	ep_start_mo 
	, concat(ep_start_year,'Q9') 
	, market_fnl
...
select count(*) from tmp_1m.kn_mbm_episode_agg6_202509; -- 2322113  2278610 2234635

insert into tmp_1m.kn_mbm_episode_agg6_202509   
select * from tmp_1m.kn_mbm_episode_agg6_ep_202509 as a;

-- Minor format adjustments
-- Considering removing because of formatting issues
alter table tmp_1m.kn_mbm_episode_agg6_202509 change data_type data_type varchar(20);
alter table tmp_1m.kn_mbm_episode_agg6_202509 change ep_start_mo ep_start_mo varchar(20);
alter table tmp_1m.kn_mbm_episode_agg6_202509 change ep_start_qtr ep_start_qtr varchar(20);
alter table tmp_1m.kn_mbm_episode_agg6_202509 change mbm_deploy_dt mbm_deploy_dt varchar(20);
alter table tmp_1m.kn_mbm_episode_agg6_202509 change claim_status claim_status varchar(20);
alter table tmp_1m.kn_mbm_episode_agg6_202509 change visit_mo visit_mo varchar(20);
alter table tmp_1m.kn_mbm_episode_agg6_202509 change category category varchar(20);

select count(*) from tmp_1m.kn_mbm_episode_agg6_202509; --2089616 2367743  2323571  2278828  2235149
...
select sum(allowed) 
from tmp_1m.kn_mbm_episode_agg6_202509
where visit_mo = '202406'; --73246897.76 73520205.78 73432103.42  73635236.05  73514079.66
...
drop table if exists tmp_1m.kn_mbm_episode_agg6_sum1_after2023_202509;
create table tmp_1m.kn_mbm_episode_agg6_sum1_after2023_202509 stored as orc as 
select 
	data_type,
	ep_start_mo,
...
select 
	...
	allowed_amt,
	mms
from tmp_1m.kn_mbm_mshp_sum1_202509
;
select count(*) from tmp_1m.kn_mbm_episode_agg6_sum1_after2023_202509; -- 72856
...
drop table if exists tmp_1m.kn_mbm_202509;
create table tmp_1m.kn_mbm_202509 as
select
	*
from tmp_1m.kn_mbm_episode_agg6_sum1_after2023_202509
union all
select 
	*
from tmp_1y.kn_mbm_episode_agg6_sum1_before2023;

select count(*) from tmp_1m.kn_mbm_202509; -- 253665
;
