...
-- QA: kn_mbm_dtl | expected growth ~1 month of members (prev run: 476794132)
select '${current_month}' as month, count(*) as n from tmp_1q.kn_mbm_dtl_${current_month}
union all
 ...
 *==============================================================================*/
create or replace table tmp_1q.kn_lopa_op_1_${current_month} as
select
 ...
 *==============================================================================*/
create or replace table tmp_1q.kn_mbm_episode_pr_${current_month} as
select
 ...
 *==============================================================================*/
create or replace table tmp_1q.kn_mbm_op_claims as
select
    e.*
...
create or replace table tmp_1q.kn_mbm_episode_1c_${current_month} as
 ...
 *==============================================================================*/
create or replace table tmp_1q.kn_mbm_episode_3_${current_month} as
select
 ...
 *==============================================================================*/
create or replace table tmp_1q.kn_mbm_episode_lag_${current_month} as
select
	a.mbi
	, a.component
	, a.id
...
select '${current_month}' as month, count(*) as n from tmp_1q.kn_mbm_episode_vst_ep_2_${current_month}
union all
select '${previous_month}' as month, count(*) as n from tmp_1q.kn_mbm_episode_vst_ep_2_${previous_month}
;
...
select sum(allowed_amt) from tmp_1q.kn_mbm_202602
where data_type = 'EPISODES'


select * from tmp_1q.kn_mbm_episode_ro_lag2_202602


select mbi, id, cmltv_episodes, ep_start_mo, visit_mo, category, claim_status, visits, episodes, allowed
from tmp_1q.kn_mbm_episode_ro_lag2_202602
where to_varchar(ep_start_dt, 'yyyymm') >= '202401'


order by mbi, ep_start_mo, visit_mo
--group by to_varchar(ep_start_dt, 'yyyymm')


select ep_start_mo, iff(mbm_deploy_dt = 'National', 'National', 'Pilot') as national_pilot_flag, category, sum(episodes)
from tmp_1q.kn_mbm_episode_ro_lag2_202602
where ep_start_mo >= '202101' and allowed = 0
group by 1, 2, 3
order by 1, 2, 3

select
  ep_start_mo
, iff(mbm_deploy_dt = 'National', 'National', 'Pilot') as national_pilot_flag
, category
, sum(iff(allowed = 0, episodes, 0)) as allowed_0_episodes_count
, sum(episodes) as total_episodes_count
, to_varchar(
        sum(iff(allowed = 0, episodes, 0))
        / nullif(sum(episodes), 0)
      , 'FM990.000%') as pct
from tmp_1q.kn_mbm_episode_ro_lag2_202602
where ep_start_mo >= '202101'
group by
  ep_start_mo
, national_pilot_flag
, category
...
select to_char(ep_start_dt, 'yyyymm'), sum(ep_flag)
from tmp_1m.knd_mbm_cosmos_csp_nice_claims_vpe_3_no_denied_202602
where to_char(ep_start_dt, 'yyyymm') >= '202401' and allowed = 0 and population = 'M&R FFS (excl. DSNP)'
group by 1


select mbi_key, visit_id, to_char(ep_start_dt, 'yyyymm'), fst_srvc_month, category_2, claim_status, ep_flag, allowed
from tmp_1m.knd_mbm_cosmos_csp_nice_claims_vpe_3_no_denied_202602
where to_varchar(ep_start_dt, 'yyyymm') >= '202401'
