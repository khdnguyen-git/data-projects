...
@set paid_thru = 202603
show variables;

select sum(allw_amt_fnl) from fichsrv.glxy_pr_f
where prov_tin = '141941024' and fst_srvc_year = '2025'

select sum(allw_amt_fnl) from fichsrv.glxy_op_f
where prov_tin = '141941024' and fst_srvc_year = '2025'

select sum(allw_amt_fnl) from fichsrv.dcsp_pr_f
where tin = '141941024' and fst_srvc_year = '2025'

select sum(allw_amt_fnl) from fichsrv.dcsp_op_f
where tin = '141941024' and fst_srvc_year = '2025'

select sum(allowed) from
tmp_1m.knd_mbm_visits_episodes_extract_${paid_thru} 
where prov_tin = '141941024'

select proc_ from fichsrv.glxy_op_f


select distinct proc_cd, proc_mod1_cd, proc_mod2_cd from fichsrv.glxy_op_f as a
where a.proc_cd in (
            -- PT/OT
