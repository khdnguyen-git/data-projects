...
select
    *
    , case
        ...
        when mnr_ffs_nodsnp_flag = 1 and mnr_dual_flag = 0 and cns_dual_flag = 0 and mnr_isnp_flag = 0 then 'M&R FFS (excl. DSNP)'
        when mnr_ffs_flag = 1 and mnr_dual_flag = 0 and cns_dual_flag = 0 and mnr_isnp_flag = 0 then 'M&R FFS'
...
select distinct fin_brand from fichsrv.tre_membership


select distinct fin_product_level_3 from fichsrv.tre_membership
where sgr_source_name = 'NICE' and fin_brand = 'M&R' and nce_tadm_dec_risk_type in ('FFS', 'PHYSICIAN') and tfm_include_flag = 1


select distinct fin_product_level_3 from fichsrv.tre_membership
where sgr_source_name = 'COSMOS' and fin_brand = 'M&R' and global_cap = 'NA' and tfm_include_flag = 1
...
select
    *
    ...
    , case
        ...
        then 1 else 0
      end as mnr_ffs_total_flag
    -- M&R FFS EXCLUDING DSNP
    , case
        when (
            (entity = 'COSMOS'
             and brand_fnl = 'M&R'
...
select
    *
    , case
        ...
        when mnr_ffs_total_flag = 1 then 'M&R FFS (total)'
        else 'N/A'
      end as population
from mm_flag;
...
select
	population
	, oah_flag
	, CnS_Dual_flag, MnR_Dual_flag, mnr_isnp_flag, mnr_ffs_total_flag, MnR_FFS_noDSNP_flag
	, sum(fin_member_cnt)
...
select
	population
	, oah_flag
	, CnS_Dual_flag, MnR_Dual_flag, mnr_isnp_flag, MnR_FFS_flag, MnR_FFS_noDSNP_flag
	, sum(fin_member_cnt)
from tmp_1m.knd_mbm_cosmos_csp_nice_mm_${current_month}
where fst_srvc_month = '202509'
group by 1,2,3,4,5,6,7
...
select total_oah_flag, cns_dual_flag, MnR_Dual_flag, institutional_flag, MnR_TOTAL_FFS_FLAG, sum(membership)
