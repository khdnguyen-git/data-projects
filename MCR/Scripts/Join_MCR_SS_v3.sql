...
select distinct
	...
	, regexp_replace(b.site_clm_aud_nbr, '^[A-Z]+0*', '') as mnr_clm_aud_nbr
...
-- Join with site_clm_aud_nbr_2 (everything before -)
with cte_join as (
select 
	a.*
	, b.*
...
select mnr_entity, mcr_routed, count(*)
from cte_join
group by mnr_entity, mcr_routed
;

--MNR_ENTITY	MCR_ROUTED	COUNT(*)
--M&R FFS	N	1
--OAH		N	2,171
--C&S DSNP	N	3,280
--N/A C&S	N	269
...
select 
	mnr_ekp
	, mnr_entity
	, mnr_site_clm_aud_nbr_1 
	, mnr_site_clm_aud_nbr_2
	, mnr_site_clm_aud_nbr_3
	, mnr_sbscr_nbr_1 
	, mnr_sbscr_nbr_2
	, mnr_site_cd_1 
	, mnr_site_cd_2
...
-- Export
drop table if exists ving_prd_trend_db.tmp_1m.kn_mcr_ss_join_cosmos_export;
create table ving_prd_trend_db.tmp_1m.kn_mcr_ss_join_cosmos_export as
select 
	mnr_entity_source
	, mnr_entity
	...
	, mnr_allw_amt_fnl 
	, mnr_month
	, mnr_ss_compare_month
	, mnr_year
	, mnr_product_level_3_fnl
	, mnr_tfm_product_new_fnl 
	, mnr_tfm_include_flag 
...
select mnr_entity, mcr_routed, count(*) from ving_prd_trend_db.tmp_1m.kn_mcr_ss_join_csp
group by all;
...
select 
	...
	, mcr_sys_class_name 
	, mcr_routed
from tmp_1m.kn_mcr_ss_join_csp
;

select mnr_entity, mcr_routed, count(*) from ving_prd_trend_db.tmp_1m.kn_mcr_ss_join_csp group by all;


drop table if exists ving_prd_trend_db.tmp_1m.kn_mcr_ss_join_csp_v2;
create table ving_prd_trend_db.tmp_1m.kn_mcr_ss_join_csp_v2 as
select 
	a.*
	, b.*
    , case when b.mcr_work_item_id_1 is null then 'N' else 'Y' end as mcr_routed
...
select 
	...
	, mnr_allw_amt_fnl 
	, 
from tmp_1m.kn_mcr_ss_join_csp_v2
;
...
select 
	...
	, mcr_priority
	, mcr_resolution_code
	, count(distinct concat(mnr_sbscr_nbr_1, mnr_site_clm_aud_nbr_1)) as n
from tmp_1m.kn_mcr_ss_join_cosmos
group by
	mnr_entity_source
	, mnr_entity
...
select distinct * from cte_union
...
select 
	...
	, mcr_priority
	, mcr_resolution_code
	, count(distinct concat(mnr_sbscr_nbr, mnr_site_clm_aud_nbr)) as n
from tmp_1m.kn_mcr_ss_join_cosmos
group by
	mnr_entity_source
	, mnr_entity
...
select member_id, work_item_id, regexp_substr(work_item_id, '^[^-]+'), u_div, claim_id, uhc_dept_received_date, uhg_received_date 
from cdw_prd_call_db.cdw_ewdo_mcr_base_view_sc.owr_x_uhgen_ewr_work
where member_id = '972669700'
order by uhc_dept_received_date 


select site_clm_aud_nbr, regexp_replace(site_clm_aud_nbr, '^[A-Z]+0*', '') as mnr_clm_aud_nbr
