...
/*Every week*/
--Find and change date: _07302025
...
describe tmp_1m.ec_avtar_23_trs;
...
describe tmp_1m.ec_ip_dataset_06182025_trs;
...
drop table tmp_1m.ec_ip_dataset_07302025_trs; 
create table tmp_1m.ec_ip_dataset_07302025_trs stored as orc as 
select 
	admit_week
	,hce_dt
	,hce_admit_month
...
drop table tmp_1m.ec_ip_dataset_07302025_2_trs; 
create table tmp_1m.ec_ip_dataset_07302025_2_trs stored as orc as 
select 
	a.*
...
from tmp_1m.ec_ip_dataset_07302025_trs as a
left join tmp_1y.hk_snf_swgbed_tins2 as b
on a.prov_tin=b.prov_tin
;

describe tmp_1m.ec_ip_dataset_07302025_3_trs;

--Step 7: Adding in a IPA/PAC split now that SWGBED is split out 
drop table tmp_1m.ec_ip_dataset_07302025_3_trs; 
create table tmp_1m.ec_ip_dataset_07302025_3_trs stored as orc as 
select 
	*
...
from  tmp_1m.ec_ip_dataset_07302025_2_trs
;


--Step 8: Roll up before join to MM 
drop table tmp_1m.ec_ip_dataset_07302025_4_trs; 
create table tmp_1m.ec_ip_dataset_07302025_4_trs stored as orc as 
select 	
	admit_week
	,hce_admit_month
--	,admit_act_qtr
...
--	,prim_diag_ahrq_genl_catgy_desc
	...
	,0 as franky_admits
	,0 as franky_allw
from tmp_1m.ec_ip_dataset_07302025_3_trs
group by 
	admit_week
	,hce_admit_month
--	,admit_act_qtr
...
drop table tmp_1m.ec_ip_dataset_07302025_mm; 
create table tmp_1m.ec_ip_dataset_07302025_mm stored as orc as 
select 
...
drop table tmp_1m.ec_ip_dataset_notif_07302025_trs;
create table tmp_1m.ec_ip_dataset_notif_07302025_trs as				
SELECT	
	*
	from tmp_1m.ec_ip_dataset_07302025_4_trs
union all select 
	* from tmp_1m.ec_ip_dataset_07302025_mm
	; 

describe tmp_1m.ec_ip_dataset_07302025_mm;
...
select distinct
	...
	,a.CLM_PL_OF_SRVC_DESC
	,a.COS_PROV_SPCL_CD
from tmp_1m.kn_ip_dataset_claims_cosmos_07302025 as a
...
drop table tmp_1m.ec_ip_dataset_claims_cosmos_07302025;
create table tmp_1m.kn_ip_dataset_claims_cosmos_07302025 as				
SELECT
/*CLAIM-RELATED FIELDS*/
	a.SITE_CLM_AUD_NBR
	,a.COMPONENT
...
select distinct year(pd_dn_ol_admit_start_dt) from tmp_1m.ec_ip_dataset_claims_cosmos_07302025;


--Step 11.5: Stack 2021 table with current table before sending through Franky
drop table tmp_1m.ec_ip_dataset_claims_cosmos_07302025_2;
create table tmp_1m.ec_ip_dataset_claims_cosmos_07302025_2 as				
SELECT
	* 
	from tmp_1m.ec_ip_dataset_claims_cosmos_07302025
	union all 
	select 
	*
...
from tmp_1m.ec_ip_dataset_claims_cosmos_07302025_2 as a
group by 
--	ADMIT_START_DT
--	,admit_end_dt 
	admit_week
...
drop table tmp_1m.ec_ip_dataset_claims_franky8_07302025 ;
CREATE TABLE tmp_1m.ec_ip_dataset_claims_franky8_07302025 as
select 
	*
...
from tmp_1m.ec_ip_dataset_claims_franky8_07302025 as a 
left join fichsrv.group_crosswalk as b
...
drop table tmp_1m.ec_ip_dataset_claims_triangle_07302025; 
create table tmp_1m.ec_ip_dataset_claims_triangle_07302025 stored as orc as 
SELECT
	000000 as admit_week
	,admit_yr_month
	,'0000' as admit_year
...
--	,0 as covid_dx_ind -- both these ind not needed
	...
	,0 as franky_admits
	,0 as frank_allowed
from tmp_1m.ec_ip_dataset_claims_cosmos_07302025_2 
group by 
	admit_yr_month
	,fst_srvc_month -- rename in notification
	,adjd_yrmonth
...
drop table tmp_1m.ec_ip_dataset_claims_fnl_trs_07302025; 
create table tmp_1m.ec_ip_dataset_claims_fnl_trs_07302025 stored as orc as 
select	
	admit_week
	,admit_yr_month
	,'0000' as admit_year
...
drop table tmp_1m.ec_ip_dataset_all_07302025_trs;
create table tmp_1m.ec_ip_dataset_all_07302025_trs as				
SELECT	
	*
	from tmp_1m.ec_ip_dataset_notif_07302025_trs
union all select 
	* 
...
select max(admit_week) from tmp_1m.ec_ip_dataset_all_07302025_TRS where ipa_pac_flag ='IPA' and loc_flag=1;
--6/18/25: 202525
--6/25/25: 202526
--7/2/25: 202527
--7/9/25: 202528
...
from tmp_1m.ec_ip_dataset_all_07302025_trs as a
...
drop table tmp_1m.ec_ip_dataset_pac_fnl_07302025 ;
create table tmp_1m.ec_ip_dataset_pac_fnl_07302025 stored as orc as
select 
	admit_type
...
describe tmp_1m.ec_ip_dataset_notif_07302025_trs;

--Step 30: LOC Valuation Pull & export 
drop table tmp_1m.ec_ip_dataset_loc_07302025 ;

drop table tmp_1m.kn_ip_dataset_loc_07302025;
create table tmp_1m.kn_ip_dataset_loc_07302025 stored as orc as
select 
	admit_week
...
drop table tmp_1m.ec_ip_dataset_LI_07302025_trs ;
create table tmp_1m.ec_ip_dataset_LI_07302025_trs stored as orc as
select 	
	admit_week
	,hce_admit_month
	,adjd_yrmonth
...
from tmp_1m.ec_ip_dataset_all_07302025_trs
...
drop table tmp_1m.ec_ip_dataset_LI_07302025_1_trs ;
create table tmp_1m.ec_ip_dataset_LI_07302025_1_trs stored as orc as
select DISTINCT 	
	a.fin_tfm_product_new
	,a.admit_type
	,a.component
...
from tmp_1m.ec_ip_dataset_LI_07302025_trs as a 
...
drop table tmp_1m.ec_ip_dataset_LI_07302025_2_trs ;
create table tmp_1m.ec_ip_dataset_LI_07302025_2_trs stored as orc as
select 
	admit_week
	,hce_admit_month
	,adjd_yrmonth
	...
	,franky_admits
	,franky_allowed
from tmp_1m.ec_ip_dataset_LI_07302025_trs
union all select 
	000000 as admit_week
	,hce_admit_month
	...
	,0 as franky_admits
	,0 as franky_allowed
from tmp_1m.ec_ip_dataset_LI_07302025_1_trs
;

--Step 34: final Roll up for export 
drop table tmp_1m.ec_ip_dataset_LI_07302025_3_trs ;
create table tmp_1m.ec_ip_dataset_LI_07302025_3_trs stored as orc as
select 
	admit_week
	,hce_admit_month
...
from tmp_1m.ec_ip_dataset_LI_07302025_2_trs
group by 
	admit_week
	,hce_admit_month
	,adjd_yrmonth
...
drop table tmp_1m.ec_ip_dataset_comp_07302025_1;
create table tmp_1m.ec_ip_dataset_comp_07302025_1 as
select 
...
drop table tmp_1m.ec_ip_dataset_comp_07302025_2a;
create table tmp_1m.ec_ip_dataset_comp_07302025_2a as
select distinct 
	admit_week
    ,hce_admit_month
from tmp_1m.ec_ip_dataset_all_07302025_trs
...
drop table tmp_1m.ec_ip_dataset_comp_07302025_2b;
create table tmp_1m.ec_ip_dataset_comp_07302025_2b as
select 
	hce_admit_month
    ,sum(membership) as membership
from tmp_1m.ec_ip_dataset_all_07302025_trs
...
drop table tmp_1m.ec_ip_dataset_comp_07302025_2;
create table tmp_1m.ec_ip_dataset_comp_07302025_2 as
select 
    ...
    ,0 as franky_allowed
    ,0 as days
from tmp_1m.ec_ip_dataset_comp_07302025_2b as a
left join tmp_1m.ec_ip_dataset_comp_07302025_2a as b
     on a.hce_admit_month = b.hce_admit_month
where b.admit_week > 0
;

--Completion Step 3: Monthly Membership
drop table tmp_1m.ec_ip_dataset_comp_07302025_3;
create table tmp_1m.ec_ip_dataset_comp_07302025_3 as
select 'Month MM'
	,0 as admit_week
	,hce_admit_month
	,'' AS service_month
...
drop table tmp_1m.ec_ip_dataset_comp_07302025_4;
create table tmp_1m.ec_ip_dataset_comp_07302025_4 as
...
drop table tmp_1m.ec_ip_dataset_comp_07302025_5;
create table tmp_1m.ec_ip_dataset_comp_07302025_5 AS
...
drop table tmp_1m.ec_ip_dataset_comp_07302025_6;
create table tmp_1m.ec_ip_dataset_comp_07302025_6 AS
...
;
t
drop table tmp_1m.ec_ip_dataset_comp_07302025_fnl;
create table tmp_1m.ec_ip_dataset_comp_07302025_fnl as
	select * from tmp_1m.ec_ip_dataset_comp_07302025_1
union all
	select * from tmp_1m.ec_ip_dataset_comp_07302025_2
union all
	select * from tmp_1m.ec_ip_dataset_comp_07302025_3
union all
	select * from tmp_1m.ec_ip_dataset_comp_07302025_4
union all
	select * from tmp_1m.ec_ip_dataset_comp_07302025_5
union all
	select * from tmp_1m.ec_ip_dataset_comp_07302025_6
union all
...
--tmp_1m.ec_ip_dataset_loc_07302025
--tmp_1m.ec_ip_dataset_LI_07302025_2_trs
--tmp_1m.ec_ip_dataset_comp_07302025_fnl
...
--set tmp_1m.ec_ip_dataset_loc_07302025
--;run;

--/*leading indicator*/
--data hcx_ec.ec_ip_dataset_7_16_2025 (compress=yes); /*CHANGE TO CURRENT DATE*/ 
--set tmp_1m.ec_ip_dataset_LI_07302025_3_trs
--;run;

--/*completion dataset*/
--data hcx_ec.ec_ip_dataset_comp_07302025_fnl (compress=yes); 
--set tmp_1m.ec_ip_dataset_comp_07302025_fnl 
--;run;
