...
------------------------------------------------------------------------------------;
create or replace table tmp_1m.kn_ip_dataset_11192025_trs as 
select 
	admit_week
	,hce_dt
	,hce_admit_month
...
create or replace table tmp_1m.kn_ip_dataset_11192025_2_trs as 
select 
	a.*
...
from tmp_1m.kn_ip_dataset_11192025_trs as a
...
create or replace table tmp_1m.kn_ip_dataset_11192025_3_trs as 
select 
	*
...
from  tmp_1m.kn_ip_dataset_11192025_2_trs
;


--Step 8: Roll up before join to MM 
create or replace table tmp_1m.kn_ip_dataset_11192025_4_trs as 
select 	
	a.admit_week
	,a.hce_admit_month
--	,a.admit_act_qtr
...
--	,a.prim_diag_ahrq_genl_catgy_desc
	...
	,0 as franky_admits
	,0 as franky_allw
from tmp_1m.kn_ip_dataset_11192025_3_trs as a
...
create table tmp_1m.kn_ip_dataset_11192025_mm as 
select 
...
--	,'MM' as prim_diag_ahrq_genl_catgy_desc 
	...
	,0 as franky_admits
	,0 as franky_allw
from tadm_tre_cpy.gl_rstd_gpsgalnce_f_202511 /**/ as a /*MAKE SURE THIS IS ENROLLMENT TABLE FOR CURRENT MONTH*/
...
create table tmp_1m.kn_ip_dataset_notif_11192025_trs as				
SELECT	
	*
	from tmp_1m.kn_ip_dataset_11192025_4_trs
union all select 
	* from tmp_1m.kn_ip_dataset_11192025_mm
	; 


--Step 30: LOC Valuation Pull & export 
create or replace table tmp_1m.kn_ip_dataset_loc_11192025 as
with aggregate as (
select 
	admit_week
...
--	,admit_act_qtr
	...
	,sum(other_ovtrns) as other_ovtrns
	,sum(membership) as membership
from tmp_1m.kn_ip_dataset_notif_11192025_trs
where ipa_pac_flag in ('IPA','MM') 
	and hce_admit_month > '202312'
	and loc_flag=1
group by  
	admit_week
	,hce_admit_month
...
select 
	*
	, case when total_oah_flag = 'OAH' then 'OAH'
	   	   when institutional_flag = 'Institutional' then 'ISNP'
	   	   ...
	   	   when mnr_oah_flag = 1 then 'M&R OAH'
		   when cns_oah_flag = 1 then 'C&S OAH'	
	else 'Others'
	end as population
from aggregate
;

select population, sum(membership)
from tmp_1m.kn_ip_dataset_loc_11192025
where admit_act_month = '202508'
group by 1 
	   	   
	   	   
-- Graph
Monthly
initial_adr, persistent_adr, 
persistency, 
MD escalation


select institutional_flag
--	admit_act_month
--	, fin_market
--	, fin_state
--	, sum(membership)
from tmp_1m.kn_ip_dataset_loc_11192025
where cns_dual_flag = 1 
group by 1,2,3
