...
@set notifications_date = 01072026
...
drop table if exists tmp_1m.ec_avtar_22_trs;
create table tmp_1m.ec_avtar_22_trs as
select 
...
left outer join
	HCE_OPS_ARCHV.hce_adr_avtar_like_2022_f as b
on  b.transplant_flag='Y'
and a.fin_mbi_hicn_fnl = b.fin_mbi_hicn_fnl
where b.fin_mbi_hicn_fnl is null and year(TRY_TO_DATE(a.transplantdate))=2022
;

--Step 2.2: Union non-notification TRS to rest of notifications
drop table if exists tmp_1m.ec_avtar_22_trs_combined;
create table tmp_1m.ec_avtar_22_trs_combined as
select 
	* from HCE_OPS_ARCHV.hce_adr_avtar_like_2022_f
union all select
...
create table tmp_1m.ec_avtar_22_1_trs as
select 
	a.*
...
drop table if exists tmp_1m.ec_avtar_22_2_trs;
create table tmp_1m.ec_avtar_22_2_trs as
select     
	*
...
drop table if exists tmp_1m.ec_avtar_22_3_trs;
create table tmp_1m.ec_avtar_22_3_trs as
select 
	a.*
...
drop table if exists tmp_1m.ec_avtar_24_25_trs;
create table tmp_1m.ec_avtar_24_25_trs as
select 
	0 as p2p_full_evertouched_cnt
	,0 as p2p_full_ovtn
	,'0' as p2p_match_ind
...
FROM
	HCE_OPS_FNL.HCEOPS_TRS_DATA_SET_FNL as a
left outer join
	HCE_OPS_FNL.HCE_ADR_AVTAR_Like_24_25_F as b
on  b.transplant_flag='Y'
and a.fin_mbi_hicn_fnl = b.fin_mbi_hicn_fnl
where b.fin_mbi_hicn_fnl is null and year(TRY_TO_DATE(a.transplantdate))>2023
;
...
drop table if exists tmp_1m.ec_avtar_24_25_trs_combined;
create table tmp_1m.ec_avtar_24_25_trs_combined as
select 
...
drop table if exists tmp_1m.ec_avtar_24_25_1;
create table tmp_1m.ec_avtar_24_25_1 as
select 
	a.*
	,TRY_TO_DATE(admission_date) as trs_admit_dt
	,TRY_TO_DATE(transplantdate) as trs_transplant_dt
...
drop table if exists tmp_1m.ec_avtar_24_25_2;
create table tmp_1m.ec_avtar_24_25_2 as
select     
	*
	,case when ip_type in ('Medical','Surgical','Transplant') and admit_dt_act is not null then admit_dt_act
		when ip_type in ('Transplant') and admit_dt_act is null and admission_date is not null then trs_admit_dt
...
drop table if exists tmp_1m.ec_avtar_24_25_3;
create table tmp_1m.ec_avtar_24_25_3 as
select 
	a.*
	,concat(lpad(year(a.hcedt),4,0),lpad(month(a.hcedt),2,0)) as hce_admit_month
	,cast(hcedt as date) as hce_dt
...
drop table if exists tmp_1m.kn_ip_dataset_${notifications_date}_trs; 
create table tmp_1m.kn_ip_dataset_${notifications_date}_trs as 
select 
	admit_week
	,hce_dt
	,hce_admit_month
...
drop table if exists tmp_1m.kn_ip_dataset_${notifications_date}_2_trs; 
create table tmp_1m.kn_ip_dataset_${notifications_date}_2_trs as 
select 
	a.*
...
from tmp_1m.kn_ip_dataset_${notifications_date}_trs as a
...
drop table if exists tmp_1m.kn_ip_dataset_${notifications_date}_3_trs; 
create table tmp_1m.kn_ip_dataset_${notifications_date}_3_trs as 
select 
	*
...
from  tmp_1m.kn_ip_dataset_${notifications_date}_2_trs
;


--Step 8: Roll up before join to MM 
drop table if exists tmp_1m.kn_ip_dataset_${notifications_date}_4_trs; 
create table tmp_1m.kn_ip_dataset_${notifications_date}_4_trs as 
select 	
	a.admit_week
	,a.hce_admit_month
--	,a.admit_act_qtr
...
--	,a.prim_diag_ahrq_genl_catgy_desc
	...
	,0 as franky_admits
	,0 as franky_allw
from tmp_1m.kn_ip_dataset_${notifications_date}_3_trs as a
...
drop table if exists tmp_1m.kn_ip_dataset_${notifications_date}_mm; 
create table tmp_1m.kn_ip_dataset_${notifications_date}_mm as 
select 
...
drop table if exists tmp_1m.kn_ip_dataset_notif_${notifications_date}_trs;
create table tmp_1m.kn_ip_dataset_notif_${notifications_date}_trs as				
SELECT	
	*
	from tmp_1m.kn_ip_dataset_${notifications_date}_4_trs
union all select 
	* from tmp_1m.kn_ip_dataset_${notifications_date}_mm
	; 


select sum(case_count)
from tmp_1m.kn_ip_dataset_notif_01072026_trs
where mnr_total_ffs_flag = 1 and hce_admit_month = '202401'
...
drop table if exists tmp_1y.ec_ip_dataset_claims_cosmos_2021;
...
/*COUNTING FIELDS
	...
	,a.ip_status_code
	,case when date_add(last_day(a.adjd_dt),-10)>=a.adjd_dt then VARCHAR(a.adjd_dt,'yyyyMM') else VARCHAR(add_months(a.adjd_dt,1),'yyyyMM') end as adjd_yrmonth
	,VARCHAR(a.pd_dn_ol_admit_start_dt,'yyyyMM') as pd_dn_ol_admit_yrmonth
...
drop table if exists tmp_1m.ec_ip_dataset_pac_navi ;
create table tmp_1m.ec_ip_dataset_pac_navi as
select 
	a.*
	,b.contract
	,b.market
...
drop table if exists tmp_1m.ec_ip_dataset_pac_navi_2 ;
create table tmp_1m.ec_ip_dataset_pac_navi_2 as
select 	
	a.*
...
drop table if exists TMP_1M.kn_IP_DATASET_LOC_${notifications_date} ;
create table TMP_1M.kn_IP_DATASET_LOC_${notifications_date} as
select 
	admit_week
...
--	,admit_act_qtr
	...
	,sum(membership) as membership
from tmp_1m.kn_IP_dataset_notif_${notifications_date}_trs
...
select 
    ...
    ,CASE WHEN admit_type = 'Surgical' THEN 'Surgical' 
    	WHEN admit_type IN ('AIR','LTAC','SNF','Swing Bed') THEN 'PAC' ELSE 'Other IPA' end as admit_type
    ,'' as cap_status
    ,sum(case_count) as case_count
    ,sum(intital_adr_cnt) as intital_adr_cnt
    ,sum(p2p_ovrtn_case_cnt) as p2p_ovrtn_case_cnt
...
drop table if exists tmp_1m.ec_ip_mm_2025;
...
/*
data HCX_EC.LOC_IP_12_24_25 (compress=yes); 
set TMP_1M.EC_IP_DATASET_LOC_12242025
;run;

data HCX_EC.EC_IP_DATASET_12_24_25 (compress=yes); 
set TMP_1M.EC_IP_DATASET_LI_12242025_3_TRS
;run;

data HCX_EC.EC_IP_DATASET_COMP_12242025_FNL (compress=yes); 
set TMP_1M.EC_IP_DATASET_COMP_12242025_FNL 
;run;
*/
