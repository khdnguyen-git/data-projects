...
drop table if exists tmp_1m.ec_avtar_23_od;
create table tmp_1m.ec_avtar_23_od as
select 
...
drop table if exists tmp_1m.ec_avtar_23_od_combined;
create table tmp_1m.ec_avtar_23_od_combined as
select 
	* from HCE_OPS_ARCHV.hce_adr_avtar_like_2023_f
union all select
	* from tmp_1m.ec_avtar_23_od
	;

--Step 2.3: Adding variable to split PAC from IPA 
drop table if exists tmp_1m.ec_avtar_23_1_od;
create table tmp_1m.ec_avtar_23_1_od as
select 
	a.*
...
from tmp_1m.ec_avtar_23_od_combined as a 
;
...
drop table if exists tmp_1m.ec_avtar_23_2_od;
create table tmp_1m.ec_avtar_23_2_od as
select     
	*
...
drop table if exists tmp_1m.ec_avtar_24_od;
create table tmp_1m.ec_avtar_24_od as
select 
	0 as p2p_full_evertouched_cnt
	,0 as p2p_full_ovtn
	,'0' as p2p_match_ind
	...
	,0 as proc_unit_cnt
	,'0' as svc_crmk_cd
	,cast(svc_start_dt as timestamp) as svc_start_dt
	,cast(svc_end_dt as timestamp) as svc_end_dt
	,'0' as svc_cat_cd
	,'0' as svc_cat_dtl_cd
	,'0' as plc_of_svc_cd
	,'0' as plc_of_svc_drv_cd
	...
	,'0' as auth_typ_cd
	,'0' as channel_cd
	,'0' as SVCDECN_CURR_GAP_OTCM_CD
	,'0' as SVCDECN_CURR_GAP_OTCM_CD_DESC
	,cast(a.admission_date as timestamp) as admit_dt_act
	,cast(admit_dt_exp as timestamp) as admit_dt_exp
	,cast(a.discharge_date as timestamp) as dschg_dt_act
	,cast(dschg_dt_exp as timestamp) as dschg_dt_exp
	,0 as bcrt_void_ind
	,'0' as ocm_migration
	,'0' as mnr_hce_drv_par_status
	,'0' as so_prov_id
...
drop table if exists tmp_1m.ec_avtar_24_od_combined;
create table tmp_1m.ec_avtar_24_od_combined as
select 
	* from HCE_OPS_ARCHV.hce_adr_avtar_like_2024_f 
union all select
	* from tmp_1m.ec_avtar_24_od
	;


--Step 3.3:Run this every week: Add week to the table with recent data that Pradeepa emails about 
drop table if exists tmp_1m.ec_avtar_24_1_od;
create table tmp_1m.ec_avtar_24_1_od as
select 
	a.*
	,TRY_TO_DATE(admission_date) as trs_admit_dt
	,TRY_TO_DATE(transplantdate) as trs_transplant_dt
...
from tmp_1m.ec_avtar_24_od_combined as a 
;
...
drop table if exists tmp_1m.ec_avtar_24_2_od;
create table tmp_1m.ec_avtar_24_2_od as
select     
	*
	,case when ip_type in ('Medical','Surgical','Transplant') and admit_dt_act is not null then admit_dt_act
		when ip_type in ('Transplant') and admit_dt_act is null and admission_date is not null then trs_admit_dt
		...
		when ip_type in ('LTAC','AIR','SNF') and admit_dt_act is not null then admit_dt_act
		when ip_type in ('LTAC','AIR','SNF') and admit_dt_act is null then admit_dt_exp else null end as hcedt
from tmp_1m.ec_avtar_24_1_od
;

--Step 3.5: Add week & hce_month variable
drop table if exists tmp_1m.ec_avtar_24_3_od;
create table tmp_1m.ec_avtar_24_3_od as
select 
	a.*
	,concat(lpad(year(a.hcedt),4,0),lpad(month(a.hcedt),2,0)) as hce_admit_month
	,cast(hcedt as date) as hce_dt
...
drop table if exists tmp_1m.ec_avtar_25_26_od;
create table tmp_1m.ec_avtar_25_26_od as
select 
	0 as p2p_full_evertouched_cnt
	,0 as p2p_full_ovtn
	,'0' as p2p_match_ind
	...
	,cast(appdecndt as timestamp) as appdecndt 
	,'0' AS APPTYPE
	,cast(APPRCVDDT as timestamp) as APPRCVDDT 
	,'0' as appoutcome
	,'0' as appmcrprevreviewfmd
	,'0' as appissuetype
	,'0' as hce_category
	...
	,a.programlvl2 as transplant_type
	,0 as medsurg_overlap_ind
	,0 AS MEMBER_APPEAL_IND
	,0 AS MEMBER_APPEAL_OVTN_IND
FROM
	HCE_OPS_FNL.HCEOPS_TRS_DATA_SET_FNL as a
left outer join
...
drop table if exists tmp_1m.ec_avtar_25_26_od_combined;
create table tmp_1m.ec_avtar_25_26_od_combined as
select 
	* from HCE_OPS_FNL.HCE_ADR_AVTAR_Like_25_26_F
union all select
	* from tmp_1m.ec_avtar_25_26_od
	;


--Step 4.3 :Run this every week: Add week to the table with recent data that Pradeepa emails about 
drop table if exists tmp_1m.ec_avtar_25_26_1_od;
create table tmp_1m.ec_avtar_25_26_1_od as
select 
	a.*
	,TRY_TO_DATE(admission_date) as trs_admit_dt
	,TRY_TO_DATE(transplantdate) as trs_transplant_dt
...
from tmp_1m.ec_avtar_25_26_od_combined as a 
;
...
drop table if exists tmp_1m.ec_avtar_25_26_2_od;
create table tmp_1m.ec_avtar_25_26_2_od as
select     
	*
	,case when ip_type in ('Medical','Surgical','Transplant') and admit_dt_act is not null then admit_dt_act
		when ip_type in ('Transplant') and admit_dt_act is null and admission_date is not null then trs_admit_dt
		...
		when ip_type in ('LTAC','AIR','SNF') and admit_dt_act is not null then admit_dt_act
		when ip_type in ('LTAC','AIR','SNF') and admit_dt_act is null then admit_dt_exp else null end as hcedt
from tmp_1m.ec_avtar_25_26_1_od
;

--Step 4.5: Add week & hce_month variable
drop table if exists tmp_1m.ec_avtar_25_26_3_od;
create table tmp_1m.ec_avtar_25_26_3_od as
select 
	a.*
	,concat(lpad(year(a.hcedt),4,0),lpad(month(a.hcedt),2,0)) as hce_admit_month
	,cast(hcedt as date) as hce_dt
...
drop table if exists tmp_1m.ec_ip_dataset_${notifications_date}_od; 
create table tmp_1m.ec_ip_dataset_${notifications_date}_od as 
select 
	admit_week
	,hce_dt
	,hce_admit_month
  ...
  --  ,prim_diag_ahrq_diag_dtl_catgy_desc 
    ...
    ,count(distinct (case when member_appeal_ind=1  then case_id  end )) as member_appeal_cnt
    ,count(distinct (case when MEMBER_APPEAL_OVTN_IND=1  then case_id  end )) as MEMBER_APPEAL_OVTN_cnt
	...
	,0 as membership
from tmp_1m.ec_avtar_25_26_3_od
where 	
...
--    ,covid_dx_ind 
 ...
 union all select 
  ...
  --  ,prim_diag_ahrq_diag_dtl_catgy_desc 
    ...
    ,count(distinct (case when P2P_full_ovtn=0 and Appeal_ovrtn_Ind=0 and MCR_Ovtrn_ind=0 and initialfulladr_cases=1 and  persistentfulladr_cases=0 then case_id end)) as Other_ovtrns
    ,0 as membership
from tmp_1m.ec_avtar_24_3_od
where 	
 	   fin_brand in ('M&R','C&S')
       and ((IP_type in ('Medical','Surgical','Transplant') and TO_VARCHAR(admit_dt_act, 'MM/dd/yyyy') is not null) or IP_type in ('LTAC','SNF','AIR'))
       and (TO_VARCHAR(hce_dt,'yyyy') in ('2024') OR  TO_VARCHAR(hce_dt,'yyyyMM')='202501')
 group by 
admit_week
	,hce_dt
	,hce_admit_month
...
--    ,covid_dx_ind 
  ...
  --  ,prim_diag_ahrq_diag_dtl_catgy_desc 
    ...
    ,count(distinct (case when P2P_full_ovtn=0 and Appeal_ovrtn_Ind=0 and MCR_Ovtrn_ind=0 and initialfulladr_cases=1 and  persistentfulladr_cases=0 then case_id end)) as Other_ovtrns
   	,0 as membership
from tmp_1m.ec_avtar_23_3_od
where 	
		fin_brand in ('M&R','C&S')
       and ((IP_type in ('Medical','Surgical','Transplant') and TO_VARCHAR(admit_dt_act, 'MM/dd/yyyy') is not null) or IP_type in ('LTAC','SNF','AIR'))
--       and TO_VARCHAR(admit_dt_act  ,'yyyy') in ('2023')		
 group by 
--		000000 as admit_week
	hce_dt
	,hce_admit_month
...
drop table if exists tmp_1m.ec_ip_dataset_${notifications_date}_2_od; 
create table tmp_1m.ec_ip_dataset_${notifications_date}_2_od as 
select 
	a.*
	,case when a.IP_type='SNF' and b.TYPE_F='IP_SWGBED' then 1 
...
from tmp_1m.ec_ip_dataset_${notifications_date}_od as a
left join TMP_1Y.HK_SWINGBED_2026 as b
on a.prov_tin=b.TIN
;


--Step 7: Adding in a IPA/PAC split now that SWGBED is split out 
drop table if exists tmp_1m.ec_ip_dataset_${notifications_date}_3_od; 
create table tmp_1m.ec_ip_dataset_${notifications_date}_3_od as 
select 
	*
...
from  tmp_1m.ec_ip_dataset_${notifications_date}_2_od
;


--Step 8: Roll up before join to MM 
drop table if exists tmp_1m.ec_ip_dataset_${notifications_date}_4_od; 
create table tmp_1m.ec_ip_dataset_${notifications_date}_4_od as 
select 	
	a.admit_week
	,a.hce_admit_month
--	,a.admit_act_qtr
...
drop table if exists tmp_1m.ec_ip_dataset_${notifications_date}_mm_od; 
create table tmp_1m.ec_ip_dataset_${notifications_date}_mm_od as 
select 
...
where fin_inc_year in ('2022','2023','2024','2025','2026')
group by 
		a.fin_inc_month
--	,a.fin_inc_qtr
	,a.fin_inc_year 
...
drop table if exists tmp_1m.ec_ip_dataset_notif_${notifications_date}_od;
create table tmp_1m.ec_ip_dataset_notif_${notifications_date}_od as				
SELECT	
	*
	from tmp_1m.ec_ip_dataset_${notifications_date}_4_od
union all select 
	* from tmp_1m.ec_ip_dataset_${notifications_date}_mm_od
	; 
...
--SERVICE FIELDS
	...
	,a.denial_f
	,case when b.drg_med_surg_txt in ('UNKNOWN','**','MED','MEDICAL','[NULL]') then 'MEDICAL' 
		when a.fnl_drg_cd is null then 'MEDICAL' 
		when b.drg_med_surg_txt in ('SURG','SURGICAL') then 'SURGICAL' else 'MEDICAL' end as med_surg
...
left join tmp_2y.ec_glxy_drg_cd_2026 as b
	...
	and CASE WHEN a.ADMIT_START_DT BETWEEN '2021-10-01' AND '2022-09-30' THEN '2022'
		WHEN a.ADMIT_START_DT BETWEEN '2022-10-01' AND '2023-09-30' THEN '2023'
		WHEN a.ADMIT_START_DT BETWEEN '2023-10-01' AND '2024-09-30' THEN '2024'
...
drop table if exists tmp_1m.ec_ip_dataset_claims_triangle_${claims_date}_od; 
create table tmp_1m.ec_ip_dataset_claims_triangle_${claims_date}_od as 
SELECT
	000000 as admit_week
	,admit_yr_month
	,'0000' as admit_year
...
--DEMOGRAPHIC FIELDS
	...
	,a.REGION_FNL
	,a.MARKET_FNL
	,a.FIN_SUBMARKET AS FIN_SUBMARKET_FNL
	,a.PMGDEC_FNL
	,a.DEC_RISK_TYPE_FNL
	,a.GROUP_IND_FNL
	,a.TFM_INCLUDE_FLAG
...
--PROVIDER FIELDS
	,a.MPIN
	,a.SRVC_PROV_ID
	,a.TIN AS PROV_TIN --1/28/26 change
	,a.FULL_NM
	,a.PROV_PRTCP_STS_CD
--SERVICE FIELDS
	,a.FNL_DRG_CD
	...
	,'0' AS DENIAL_F --1/28/26 CHANGE due TO inability TO find the equivalent IN NEW NICE table
...
--COUNTING FIELDS
	,a.FNL_IPRVUWGT
	,a.SBMT_CHRG_AMT
	,a.ALLW_AMT AS ALLW_AMT_FNL --1/28/26 change
	,a.NET_PD_AMT AS NET_PD_AMT_FNL --1/28/26 change
	,a.TADM_ADMITS
	,a.TADM_QTYDAYS
...
from FICHSRV.DCSP_IP_ADMIT_F as a
...
drop table if exists tmp_1m.ec_ip_dataset_claims_od; 
create table tmp_1m.ec_ip_dataset_claims_od as 
select
	a.admit_week
	,a.tadm_admit_type
	,a.admit_yr_month 
...
drop table if exists tmp_1m.ec_ip_dataset_claims_fnl_od_${claims_date}; 
create table tmp_1m.ec_ip_dataset_claims_fnl_od_${claims_date} as 
select	
	admit_week
	,admit_yr_month
	,'0000' as admit_year
...
from  tmp_1m.ec_ip_dataset_claims_cosmos_franky
group by 
	admit_week
	,admit_yr_month
--	,admit_qtr
...
drop table if exists tmp_1m.ec_ip_dataset_all_${notifications_date}_od;
create table tmp_1m.ec_ip_dataset_all_${notifications_date}_od as				
SELECT	
	*
	from tmp_1m.ec_ip_dataset_notif_${notifications_date}_od
union all select 
	* 
	from tmp_1m.ec_ip_dataset_claims_fnl_od_${claims_date} 
union all select 
	* 
...
drop table if exists TMP_1M.EC_IP_DATASET_LOC_${notifications_date}_OD ;
create table TMP_1M.EC_IP_DATASET_LOC_${notifications_date}_OD as
select 
	admit_week
...
drop table if exists tmp_1m.ec_ip_dataset_LI_${notifications_date}_od ;
create table tmp_1m.ec_ip_dataset_LI_${notifications_date}_od as
select 	
	admit_week
	,hce_admit_month
	,adjd_yrmonth
...
drop table if exists tmp_1m.ec_ip_dataset_LI_${notifications_date}_1_od ;
create table tmp_1m.ec_ip_dataset_LI_${notifications_date}_1_od as
select DISTINCT 	
	a.fin_tfm_product_new
	,a.admit_type
	,a.component
...
from tmp_1m.ec_ip_dataset_LI_${notifications_date}_od as a 
...
drop table if exists tmp_1m.ec_ip_dataset_LI_${notifications_date}_2_od ;
create table tmp_1m.ec_ip_dataset_LI_${notifications_date}_2_od as
select 
	admit_week
	,hce_admit_month
	,adjd_yrmonth
	...
	,franky_admits
	,franky_allowed
from tmp_1m.ec_ip_dataset_LI_${notifications_date}_od
union all select 
	000000 as admit_week
	,hce_admit_month
	...
	,0 as franky_admits
	,0 as franky_allowed
from tmp_1m.ec_ip_dataset_LI_${notifications_date}_1_od
;

--Step 34: final Roll up for export 
drop table if exists TMP_1M.EC_IP_DATASET_LI_${notifications_date}_3_OD ;
create table TMP_1M.EC_IP_DATASET_LI_${notifications_date}_3_od as
select 
	admit_week
	,hce_admit_month
...
from tmp_1m.ec_ip_dataset_LI_${notifications_date}_2_od
group by 
	admit_week
	,hce_admit_month
	,adjd_yrmonth
...
drop table if exists tmp_1m.ec_ip_dataset_comp_${notifications_date}_1_od;
create table tmp_1m.ec_ip_dataset_comp_${notifications_date}_1_od as
select 
...
drop table if exists tmp_1m.ec_ip_dataset_comp_${notifications_date}_2a_od;
create table tmp_1m.ec_ip_dataset_comp_${notifications_date}_2a_od as
select distinct 
	admit_week
    ,hce_admit_month
from tmp_1m.ec_ip_dataset_all_${notifications_date}_od
...
drop table if exists tmp_1m.ec_ip_dataset_comp_${notifications_date}_2b_od;
create table tmp_1m.ec_ip_dataset_comp_${notifications_date}_2b_od as
select 
	hce_admit_month
    ,sum(membership) as membership
from tmp_1m.ec_ip_dataset_all_${notifications_date}_od
...
drop table if exists tmp_1m.ec_ip_dataset_comp_${notifications_date}_2_od;
create table tmp_1m.ec_ip_dataset_comp_${notifications_date}_2_od as
select 
    ...
    ,0 as franky_allowed
    ,0 as franky_days
from tmp_1m.ec_ip_dataset_comp_${notifications_date}_2b_od as a
left join tmp_1m.ec_ip_dataset_comp_${notifications_date}_2a_od as b
     on a.hce_admit_month = b.hce_admit_month
where b.admit_week > 0
;

--Completion Step 3: Monthly Membership
drop table if exists tmp_1m.ec_ip_dataset_comp_${notifications_date}_3_od;
create table tmp_1m.ec_ip_dataset_comp_${notifications_date}_3_od as
...
drop table if exists tmp_1m.ec_ip_dataset_comp_${notifications_date}_4_od;
create table tmp_1m.ec_ip_dataset_comp_${notifications_date}_4_od as
...
drop table if exists tmp_1m.ec_ip_dataset_comp_${notifications_date}_5_od;
create table tmp_1m.ec_ip_dataset_comp_${notifications_date}_5_od AS
...
drop table if exists tmp_1m.ec_ip_dataset_comp_${notifications_date}_6_od;
create table tmp_1m.ec_ip_dataset_comp_${notifications_date}_6_od AS
...
select 'Service Month' as comp_type
	,0 as admit_week
	,hce_admit_month
	,fst_srvc_month as service_month
	,'' AS adjd_yrmonth
    ...
    ,0 as membership
    ,0 as franky_admits
	,sum(allowed) as franky_allowed
	,0 as franky_days
from tmp_1m.ec_ip_dataset_all_${notifications_date}_od
	where component = 'Triangle' 
		AND ipa_pac_flag = 'PAC'
		and leading_ind_pop = 1
		and adjd_yrmonth not in ('','MM')
		AND admit_type not in ('Other')
		AND hce_admit_month IS NOT NULL
group by hce_admit_month
	,fst_srvc_month
	,fin_tfm_product_new
	,ipa_pac_flag
...
select distinct fin_mbi_hicn_fnl
	...
	,fin_inc_month as enroll_month
	,fin_tfm_product_new
from HCE_OPS_ARCHV.GL_RSTD_GPSGALNCE_F_202601 as a
where fin_inc_year in ('2025','2026')
 ...
 select distinct fin_mbi_hicn_fnl
	...
	,fin_inc_month as enroll_month
	,fin_tfm_product_new
from HCE_OPS_ARCHV.GL_RSTD_GPSGALNCE_F_202602 as a
where fin_inc_year in ('2025','2026')
	and fin_brand='M&R'
	AND global_cap='NA'
	AND sgr_source_name='COSMOS'
...
where fin_inc_year in ('2025','2026')
 ...
 select distinct fin_mbi_hicn_fnl
	...
	,fin_inc_month as enroll_month
	,fin_tfm_product_new
from HCE_OPS_ARCHV.GL_RSTD_GPSGALNCE_F_202607 as a
where fin_inc_year in ('2025','2026')
	and fin_brand='M&R'
	AND global_cap='NA'
	AND sgr_source_name='COSMOS'
 ...
 select distinct fin_mbi_hicn_fnl
	...
	,fin_inc_month as enroll_month
	,fin_tfm_product_new
from HCE_OPS_ARCHV.GL_RSTD_GPSGALNCE_F_202608 as a
where fin_inc_year in ('2025','2026')
	and fin_brand='M&R'
	AND global_cap='NA'
	AND sgr_source_name='COSMOS'
 ...
 select distinct fin_mbi_hicn_fnl
	...
	,fin_inc_month as enroll_month
	,fin_tfm_product_new
from HCE_OPS_ARCHV.GL_RSTD_GPSGALNCE_F_202609 as a
where fin_inc_year in ('2025','2026')
	and fin_brand='M&R'
	AND global_cap='NA'
	AND sgr_source_name='COSMOS'
 ...
 select distinct fin_mbi_hicn_fnl
	...
	,fin_inc_month as enroll_month
	,fin_tfm_product_new
from HCE_OPS_ARCHV.GL_RSTD_GPSGALNCE_F_202610 as a
where fin_inc_year in ('2025','2026')
	and fin_brand='M&R'
	AND global_cap='NA'
	AND sgr_source_name='COSMOS'
 ...
 select distinct fin_mbi_hicn_fnl
	...
	,fin_inc_month as enroll_month
	,fin_tfm_product_new
from HCE_OPS_ARCHV.GL_RSTD_GPSGALNCE_F_202611 as a
where fin_inc_year in ('2025','2026')
	and fin_brand='M&R'
	AND global_cap='NA'
	AND sgr_source_name='COSMOS'
 ...
 select distinct fin_mbi_hicn_fnl
	...
	,fin_inc_month as enroll_month
	,fin_tfm_product_new
from HCE_OPS_ARCHV.GL_RSTD_GPSGALNCE_F_202612 as a
where fin_inc_year in ('2025','2026')
	and fin_brand='M&R'
	AND global_cap='NA'
	AND sgr_source_name='COSMOS'
...
drop table if exists tmp_1m.ec_ip_mm_joiner2_od;
create table tmp_1m.ec_ip_mm_joiner2_od as
...
drop table if exists tmp_1m.ec_ip_mm_leaver2_od;
create table tmp_1m.ec_ip_mm_leaver2_od as
...
drop table if exists TMP_1M.EC_IP_COMP_${notifications_date}_OD;
create table TMP_1M.EC_IP_COMP_${notifications_date}_OD as
	select * from tmp_1m.ec_ip_dataset_comp_${notifications_date}_1_od
union all
	select * from tmp_1m.ec_ip_dataset_comp_${notifications_date}_2_od
union all
	select * from tmp_1m.ec_ip_dataset_comp_${notifications_date}_3_od
union all
	select * from tmp_1m.ec_ip_dataset_comp_${notifications_date}_4_od
union all
	select * from tmp_1m.ec_ip_dataset_comp_${notifications_date}_5_od
union all
	select * from tmp_1m.ec_ip_dataset_comp_${notifications_date}_6_od
union all
	select * from tmp_1m.ec_ip_dataset_comp_${notifications_date}_7_od
union all
	select * from tmp_1m.ec_ip_mm_joiner2_od
union all
	select * from tmp_1m.ec_ip_mm_leaver2_od
;
