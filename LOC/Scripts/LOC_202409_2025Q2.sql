...
select * from tmp_1m.ec_avtar_24_25_3 limit 2
...
select 
	fin_mbi_hicn_fnl
	, fin_mbi_hicn_fnl as medicare_id
...
drop table tmp_1m.kn_ip_dataset_claims_cosmos_09242025;
create table tmp_1m.kn_ip_dataset_claims_cosmos_09242025 as				
SELECT
/*CLAIM-RELATED FIELDS*/
	a.SITE_CLM_AUD_NBR
	,a.COMPONENT
...
select distinct year(pd_dn_ol_admit_start_dt) from tmp_1m.kn_ip_dataset_claims_cosmos_09242025;


--Step 11.5: Stack 2021 table with current table before sending through Franky
drop table tmp_1m.kn_ip_dataset_claims_cosmos_09242025_2;
create table tmp_1m.kn_ip_dataset_claims_cosmos_09242025_2 as				
SELECT
	* 
	from tmp_1m.kn_ip_dataset_claims_cosmos_09242025
	union all 
	select 
	*
...
--	,sum(days) as days
from tmp_1m.kn_ip_dataset_claims_cosmos_09242025_2 as a
group by 
--	ADMIT_START_DT
--	,admit_end_dt 
	admit_week
...
drop table tmp_1m.kn_ip_dataset_claims_franky8_09242025 ;
CREATE TABLE tmp_1m.kn_ip_dataset_claims_franky8_09242025 as
select 
	*
	,d.collection as Hospital_Group
...
--	,count(distinct concat(a.dt,a.pd_dn_ol_admitid)) as length_of_stay
	...
	,0 as days
from tmp_1m.kn_ip_dataset_claims_franky8_09242025 as a 
left join fichsrv.group_crosswalk as b
...
drop table tmp_1m.kn_ip_dataset_claims_triangle_09242025; 
create table tmp_1m.kn_ip_dataset_claims_triangle_09242025 stored as orc as 
SELECT
	000000 as admit_week
	,admit_yr_month
	,'0000' as admit_year
...
--	,0 as covid_dx_ind -- both these ind not needed
	...
	,0 as franky_admits
	,0 as frank_allowed
from tmp_1m.kn_ip_dataset_claims_cosmos_09242025_2 
group by 
	admit_yr_month
	,fst_srvc_month -- rename in notification
	,adjd_yrmonth
...
drop table tmp_1m.kn_ip_dataset_claims_fnl_trs_09242025; 
create table tmp_1m.kn_ip_dataset_claims_fnl_trs_09242025 stored as orc as 
select	
	admit_week
	,admit_yr_month
	,'0000' as admit_year
...
drop table tmp_1m.kn_ip_dataset_all_09242025_trs;
create table tmp_1m.kn_ip_dataset_all_09242025_trs as				
SELECT	
	*
...
union all select 
	* 
	...
	/*For 9/17 update this should be 09032025 - will remain until September claims*/ 
union all select 
	* 
...
select max(admit_week) from tmp_1m.kn_ip_dataset_all_09242025_TRS where ipa_pac_flag ='IPA' and loc_flag=1;
--6/18/25: 202525
--6/25/25: 202526
--7/2/25: 202527
--7/9/25: 202528
...
from tmp_1m.kn_ip_dataset_all_09242025_trs as a
...
drop table tmp_1m.kn_ip_dataset_pac_fnl_09242025 ;
create table tmp_1m.kn_ip_dataset_pac_fnl_09242025 stored as orc as
select 
	admit_type
...
drop table tmp_1m.kn_ip_dataset_loc_09242025_la_2024_2025q2;
create table tmp_1m.kn_ip_dataset_loc_09242025_la_2024_2025q2 stored as orc as
select 
	fin_mbi_hicn_fnl
	, medicare_id
	,admit_week
...
select count(*) from tmp_1m.kn_ip_dataset_loc_09242025_la_2024_2025q2

select 
	admit_act_month
	, sum(membership) as mm_1
	, count(distinct fin_mbi_hicn_fnl) as mm_2
from tmp_1m.kn_ip_dataset_loc_09242025_la_2024_2025q2
group by
	admit_act_month
...
drop table tmp_1m.kn_ip_dataset_LI_09242025_trs ;
create table tmp_1m.kn_ip_dataset_LI_09242025_trs stored as orc as
select 	
	admit_week
	,hce_admit_month
	,adjd_yrmonth
...
from tmp_1m.kn_ip_dataset_all_09242025_trs
...
drop table tmp_1m.kn_ip_dataset_LI_09242025_1_trs ;
create table tmp_1m.kn_ip_dataset_LI_09242025_1_trs stored as orc as
select DISTINCT 	
	a.fin_tfm_product_new
	,a.admit_type
	,a.component
...
from tmp_1m.kn_ip_dataset_LI_09242025_trs as a 
...
drop table tmp_1m.kn_ip_dataset_LI_09242025_2_trs ;
create table tmp_1m.kn_ip_dataset_LI_09242025_2_trs stored as orc as
select 
	admit_week
	,hce_admit_month
	,adjd_yrmonth
	...
	,franky_admits
	,franky_allowed
from tmp_1m.kn_ip_dataset_LI_09242025_trs
union all select 
	000000 as admit_week
	,hce_admit_month
	...
	,0 as franky_admits
	,0 as franky_allowed
from tmp_1m.kn_ip_dataset_LI_09242025_1_trs
;

--Step 34: final Roll up for export 
drop table tmp_1m.kn_ip_dataset_LI_09242025_3_trs ;
create table tmp_1m.kn_ip_dataset_LI_09242025_3_trs stored as orc as
select 
	admit_week
	,hce_admit_month
...
from tmp_1m.kn_ip_dataset_LI_09242025_2_trs
group by 
	admit_week
	,hce_admit_month
	,adjd_yrmonth
...
drop table tmp_1m.kn_ip_dataset_comp_09242025_1;
create table tmp_1m.kn_ip_dataset_comp_09242025_1 as
select 
...
drop table tmp_1m.kn_ip_dataset_comp_09242025_2a;
create table tmp_1m.kn_ip_dataset_comp_09242025_2a as
select distinct 
	admit_week
    ,hce_admit_month
from tmp_1m.kn_ip_dataset_all_09242025_trs
...
drop table tmp_1m.kn_ip_dataset_comp_09242025_2b;
create table tmp_1m.kn_ip_dataset_comp_09242025_2b as
select 
	hce_admit_month
    ,sum(membership) as membership
from tmp_1m.kn_ip_dataset_all_09242025_trs
...
drop table tmp_1m.kn_ip_dataset_comp_09242025_2;
create table tmp_1m.kn_ip_dataset_comp_09242025_2 as
select 
    ...
    ,0 as franky_allowed
    ,0 as franky_days
from tmp_1m.kn_ip_dataset_comp_09242025_2b as a
left join tmp_1m.kn_ip_dataset_comp_09242025_2a as b
     on a.hce_admit_month = b.hce_admit_month
where b.admit_week > 0
;

--Completion Step 3: Monthly Membership
drop table tmp_1m.kn_ip_dataset_comp_09242025_3;
create table tmp_1m.kn_ip_dataset_comp_09242025_3 as
select 'Month MM'
	,0 as admit_week
	,hce_admit_month
	,'' AS service_month
...
drop table tmp_1m.kn_ip_dataset_comp_09242025_4;
create table tmp_1m.kn_ip_dataset_comp_09242025_4 as
...
drop table tmp_1m.kn_ip_dataset_comp_09242025_5;
create table tmp_1m.kn_ip_dataset_comp_09242025_5 AS
...
drop table tmp_1m.kn_ip_dataset_comp_09242025_6;
create table tmp_1m.kn_ip_dataset_comp_09242025_6 AS
...
drop table tmp_1m.kn_ip_dataset_comp_09242025_fnl;
create table tmp_1m.kn_ip_dataset_comp_09242025_fnl as
	select * from tmp_1m.kn_ip_dataset_comp_09242025_1
union all
	select * from tmp_1m.kn_ip_dataset_comp_09242025_2
union all
	select * from tmp_1m.kn_ip_dataset_comp_09242025_3
union all
	select * from tmp_1m.kn_ip_dataset_comp_09242025_4
union all
	select * from tmp_1m.kn_ip_dataset_comp_09242025_5
union all
	select * from tmp_1m.kn_ip_dataset_comp_09242025_6
union all
...
--tmp_1m.kn_ip_dataset_LI_09242025_2_trs
--tmp_1m.kn_ip_dataset_comp_09242025_fnl

--
--libname HCX_EC "/hpsasfin/int/projects/hcemrn/ec/prod/data/";

--/*LOC valuation*/
--data hcx_ec.LOC_IP_9_24_25 (compress=yes); /*CHANGE TO CURRENT DATE*/
--set tmp_1m.kn_ip_dataset_loc_09242025
--;run;

--/*leading indicator*/
--data hcx_ec.ec_ip_dataset_9_24_2025 (compress=yes); /*CHANGE TO CURRENT DATE*/ 
--set tmp_1m.kn_ip_dataset_LI_09242025_3_trs
--;run;

--/*completion dataset*/
--data hcx_ec.ec_ip_dataset_comp_09242025_fnl (compress=yes); 
--set tmp_1m.kn_ip_dataset_comp_09242025_fnl 
--;run;




select * from tmp_1m.ec_a
